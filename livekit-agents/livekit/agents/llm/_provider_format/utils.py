...
def group_tool_calls(chat_ctx: llm.ChatContext) -> list[_ChatItemGroup]:
    ...
    seen_ids: set[str] = set()  # prevent duplicate items altogether

    for item in chat_ctx.items:
        if item.id in seen_ids:
            logger.debug("Skipping duplicate chat item", extra={"item_id": item.id})
            continue
        ...
        if (item.type == "message" and item.role == "assistant") or item.type == "function_call":
            ...
            if group_id not in item_groups:
                ...
                # avoid multiple messages in the same group
                if item.type == "message" and item_groups[group_id].message is not None:
                    logger.debug(
                        "Skipping extra assistant message in group",
                        extra={"group_id": group_id, "item_id": item.id},
                    )
                    continue
                item_groups[group_id].add(item)
        elif item.type == "function_call_output":
